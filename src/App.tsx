...
export function App() {
  const [appMode, setAppMode] = useState<'dashboard' | 'builder' | 'showcase' | 'presenter'>('dashboard');
  ...
  const [uploadedMediaUrl, setUploadedMediaUrl] = useState<string>('https://images.unsplash.com/photo-1614728894747-a83421e2b9c9?w=600&auto=format&fit=crop&q=80');
  ...
  const [tiles, setTiles] = useState<{ id: string; letter: string; state: TileTokenState }[]>([
  ...
  const cycleTileState = (index: number) => {
    ...
    setTiles((prev) => {
      ...
      const states: TileTokenState[] = ['idle', 'selected', 'correct', 'wrong', 'disabled'];
  ...
  const handleSimulateAnswer = (isCorrect: boolean) => {
    if (isCorrect) {
      ...
      setFeedbackState({
        isOpen: true,
        isCorrect: true,
        message: 'Luar biasa! Sound feedback chime & bottom sheet membal aktif.',
      });
    } else {
      playWrong();
      setFeedbackState({
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between">
          <div className="flex items-center gap-2 py-2">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs sm:text-sm font-black flex items-center gap-2 transition-all ${
                appMode === 'builder'
                  ? 'bg-duo-orange text-white shadow-sm border-b-2 border-b-duo-orange-border'
                  : 'text-[#777777] hover:text-duo-dark hover:bg-slate-100'
              }`}
            >
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs sm:text-sm font-black flex items-center gap-2 transition-all ${
                appMode === 'presenter'
                  ? 'bg-duo-yellow-border text-white shadow-sm border-b-2 border-b-amber-600'
                  : 'text-[#777777] hover:text-duo-dark hover:bg-slate-100'
              }`}
            >
              <Trophy className="w-4 h-4" />
              Presenter Layar Penuh
            </button>
          </div>
        ...
        <main className="flex-1 max-w-4xl w-full mx-auto p-4 sm:p-6 md:p-8 flex flex-col gap-6">
          ...
          <DuoCard elevated className="p-4 flex items-center gap-4">
            <div className="flex-1">
              <div className="flex justify-between items-center mb-1 text-xs font-black uppercase text-[#777777]">
                <span>Progres Sesi ({currentStep}/{totalSteps})</span>
          ...
          <DuoCard elevated className="flex flex-col gap-4">
            ...
            {user ? (
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 bg-duo-green-light/40 border-2 border-duo-green/40 rounded-2xl">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-2xl bg-duo-green text-white flex items-center justify-center font-black text-lg shadow-sm">
                    {profile?.full_name ? profile.full_name[0]?.toUpperCase() : 'G'}
                  </div>
                  <div>
              ...
              <div className="flex flex-col gap-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 bg-slate-50 border-2 border-duo-gray rounded-2xl">
                  <div>
                    ...
                    <p className="text-xs font-semibold text-[#777777] mt-0.5">
                      Siswa menjawab langsung di perangkat kelas. Guru dapat masuk untuk membuat & mengedit kuis.
                    </p>
                  </div>
                ...
                {/* Inline Auth Views */}
                {authView === 'login' && (
                  <div className="relative border-2 border-duo-blue/30 rounded-3xl p-4 bg-white">
                    <button
                      type="button"
                      onClick={() => setAuthView('none')}
                      className="absolute top-4 right-4 text-xs font-bold text-slate-400 hover:text-duo-dark"
                    >
                      Tutup ✕
                    </button>
                    <LoginPage
          ...
          <DuoCard elevated className="flex flex-col gap-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              ...
              <Badge variant="blue">{registeredPlugins.length} Plugin Terdaftar</Badge>
            </div>

            <p className="text-xs sm:text-sm font-semibold text-[#777777]">
              Setiap tipe soal mini-game diisolasi dalam satu modul plugin independen yang mengimplementasikan
              kontrak <code className="bg-slate-100 px-1 py-0.5 rounded text-xs font-bold text-duo-dark">QuestionPlugin&lt;TContent, TAnswer&gt;</code>.
            </p>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-extrabold text-sm text-duo-dark truncate">{plugin.title}</h4>
                      <p className="text-xs font-semibold text-slate-400 truncate">{plugin.description}</p>
                    </div>
          ...
          <DuoCard elevated className="flex flex-col gap-4">
            ...
            <p className="text-xs sm:text-sm font-semibold text-[#777777]">
              Gambar dioptimasi 100% di peramban pengguna sebelum push ke Supabase Storage (bucket <code className="bg-slate-100 px-1 py-0.5 rounded text-xs font-bold text-duo-dark">quiz-media</code>).
            </p>
          ...
          <DuoCard elevated className="flex flex-col gap-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              ...
              <TactileButton
                variant="outline"
                size="sm"
                icon={isMuted ? <VolumeX className="w-4 h-4 text-duo-red" /> : <Volume2 className="w-4 h-4 text-duo-green" />}
                onClick={() => toggleMute()}
          ...
          <DuoCard elevated className="flex flex-col items-center text-center gap-4">
            <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-semibold text-[#777777] max-w-md">
              Klik tiap balok huruf di bawah untuk mendengarkan audio pop dan menguji mikro-animasi pegas (*spring physics*).
            </p>
