...
import { Shuffle, HelpCircle, Check, Sparkles, MoveHorizontal } from 'lucide-react';
...
export const AnagramPlayer: React.FC<PlayerProps<AnagramContent, AnagramAnswer>> = ({
  content,
  submittedAnswer,
  onAnswerSubmit,
  isEvaluating = false,
  ...
  const scrambleTiles = useCallback((tiles: AnagramTile[]): AnagramTile[] => {
    ...
    do {
      ...
      attempts++;
    } while (copy.map((t) => t.letter).join('') === targetWord && attempts < 10);
    return copy;
  }, [targetWord]);
    ...
    <div className="flex flex-col items-center w-full max-w-2xl mx-auto py-4 px-2 select-none">
      ...
      <div className="flex flex-col items-center gap-1.5 mb-6 text-center">
        ...
        <p className="flex items-center gap-1.5 text-xs font-bold text-slate-400">
          ...
          <span>Tahan dan geser ubin secara horizontal untuk mengubah urutan</span>
        </p>
      </div>
      ...
      >
        {items.map((item, index) => (
          <Reorder.Item
            ...
            whileHover={!isInteractionDisabled ? { scale: 1.05, y: -2 } : undefined}
            whileDrag={{
              scale: 1.15,
              y: -8,
            ...
            className={cn(
              ...
              !isInteractionDisabled ? 'cursor-grab active:cursor-grabbing touch-none' : 'cursor-default',
      ...
      {isAnswered && (
        ...
        >
          ...
          >
            {isCorrect ? <Check className="w-6 h-6" strokeWidth={3} /> : <Sparkles className="w-5 h-5 text-white" />}
          </div>
          <div>
            <h4 className="text-xs font-black uppercase tracking-wider">
              {isCorrect ? 'Anagram Berhasil Dipecahkan!' : 'Susunan Kata Belum Tepat'}
            </h4>
