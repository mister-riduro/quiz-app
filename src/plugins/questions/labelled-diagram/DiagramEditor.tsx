...
import { MapPin, HelpCircle, Trash2, Crosshair, RotateCcw } from "lucide-react";
    ...
    <DuoCard elevated className="flex flex-col gap-6 text-left p-6 sm:p-7">
      ...
      <div className="flex flex-col gap-3">
        ...
        <ImageUploader
          ...
          label="Unggah Gambar Diagram Soal (JPG, PNG, WebP)"
        />
      </div>
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center justify-between flex-wrap gap-2">
          ...
          <span className="text-xs font-bold text-slate-400">
            Klik gambar untuk menancapkan pin baru, geser pin untuk mengubah
            posisi
          </span>
        </div>
          ...
          {labels.length === 0 && (
            <div className="absolute inset-0 bg-black/20 backdrop-blur-[1px] flex flex-col items-center justify-center p-6 text-center pointer-events-none">
              ...
              <p className="text-white font-black text-base drop-shadow-md">
                Klik di mana saja pada gambar diagram untuk menancapkan pin
                target
              </p>
              <p className="text-white/80 font-bold text-xs mt-1">
                Koordinat akan otomatis disimpan dalam nilai persentase
                responsif (0–100%)
              </p>
            </div>
          )}
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-xs font-black uppercase tracking-wider text-slate-600">
              Kunci Jawaban Pin ({labels.length})
            </h4>
          </div>
        ...
        {labels.length === 0 ? (
          <div className="py-6 px-4 text-center border-2 border-dashed border-duo-gray rounded-xl bg-white">
            <span className="text-xs font-bold text-slate-400 italic">
              Belum ada pin target. Klik area gambar diagram di atas untuk
              menambahkan.
            </span>
          </div>
        ) : (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <input
                      ...
                      value={pin.text}
                      onChange={(e) =>
                        handleLabelTextChange(pin.id, e.target.value)
                      }
