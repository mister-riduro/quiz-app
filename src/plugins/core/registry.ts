...
export class PluginRegistry {
  ...
  public registerPlugin(plugin: QuestionPlugin<any, any>): void {
    if (this.plugins.has(plugin.type) && this.plugins.get(plugin.type) !== plugin) {
      console.warn(
    ...
    if (!plugin) {
      ...
      throw new Error(
        `[EduPlay PluginRegistry Error] Tipe soal "${type}" belum terdaftar! Plugin yang tersedia saat ini: [${
          available.length > 0 ? available.join(', ') : 'belum ada plugin terdaftar'
...
export const registerPlugin = (plugin: QuestionPlugin<any, any>) => pluginRegistry.registerPlugin(plugin);
export const getPlugin = <TContent = any, TAnswer = any>(type: QuestionTypeEnum) =>
